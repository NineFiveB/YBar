...
`ybar autostart enable` registers the bar as a login LaunchAgent (KeepAlive on
crash, config discovered at each start). See [docs/INSTALL.md](docs/INSTALL.md)
for the release-zip route, first-run privacy-permission walkthrough, stable
local signing (keeps TCC grants across rebuilds), and what autostart writes.
