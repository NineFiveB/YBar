...
| Private API required for core operation | **Public API only** — the v1 plan kept a slot for opt-in SkyLight modules behind protocols; that slot was closed, and no engine source or shipped binary links or dlsyms a private symbol. The engine's one undocumented-ABI read is `WifiScan`'s peek at `CWNetworkProfile`'s private `_isPersonalHotspot` ivar through the public ObjC runtime (`class_getInstanceVariable` + `ivar_getOffset`, presence- and type-checked; on failure the Wi-Fi popup lists no saved hotspots). The one private-API *user* in the repo sits outside the engine: the opt-in `examples/sketchybar-port/helpers/menus`, built only by `make helpers` and shipped by no install (see VISION.md) | Tahoe churn firewall (macOS 26 already forced sketchybar into dlsym shims) |
...
- `main.swift`: the local verbs come first — `ybar start|stop|restart|status`, `ybar theme …` and `ybar autostart …` (`LocalVerbs.run`; they work with no daemon, and the only socket traffic they generate is the `--exit` `stop` sends, the `--reload` `theme use` sends and the `--ping` the readiness waits use); `-h/--help` and `-v/--version` print locally; otherwise if argv has domain args → `CLIClient.runIfClient` → `SocketClient.send(argv)` → print reply, exit (`[!]`-prefixed reply → stderr, exit 1 — sketchybar convention). Else (no args, or `-c <path>`) daemon.
- Process control (`LocalVerbs`): `start` launches YBar.app through `open -n -g` so the daemon keeps the bundle's TCC identity (`-n` because LaunchServices otherwise reactivates an existing instance and silently drops everything after `--args`) — unless a login job owns the bar, in which case the job is `launchctl kickstart`ed, or bootstrapped again from its plist after a `stop` that had to boot it out. `stop` sends `--exit` and waits for the socket file to go; `restart` on a launchd-owned bar asks it to quit cleanly first and then kickstarts the job. `launchctl print`'s exit code gives loaded / not loaded / no GUI domain, and its `pid = N` line whether the job's process is alive: a live process that does not answer the socket is reported as such by `status` and `stop` (which boots it out at once, the one stop launchd will not undo), while `start` and `restart` first give it 15 s to bind — a bar in its first seconds of boot looks the same — and only then replace it with `kickstart -k` (a SIGKILL, so no teardown); a plain kickstart whose bar has not answered within the throttle (30 s + 15 s) is kicked once more with `-k` and given a bar's boot allowance (15 s, 60 s in all) before the failure names the log — `StandardErrorPath` read from the job's own plist, since a hand-written one can put it anywhere. `stop` also reads the plist's `KeepAlive`: `SuccessfulExit = false` (what `enable` writes) or `Crashed = true` lets a clean exit stick; anything else — `true`, `SuccessfulExit = true`, `Crashed = false`, a `PathState`/`NetworkState`/`OtherJob*` condition, an empty dictionary (launchd ORs the keys) — would bring the bar straight back, so the job is booted out and the message says the plist remains for `start` or the next login — unless launchd holds a persistent disable override for the label (Login Items, off), in which case `stop` says the next login will not load it, as `status` does. `start -c` against a loaded job says the job stays down and `-c` applies to that unmanaged run only. `autostart enable|disable|status` writes, boots and reports the `~/Library/LaunchAgents/com.ybar.YBar.plist` agent (`RunAtLoad`, `KeepAlive.SuccessfulExit=false`, `ProcessType=Interactive`, `LimitLoadToSessionType=Aqua`, `AssociatedBundleIdentifiers`, `ThrottleInterval=30`, `StandardErrorPath`). Every verb that launches, writes or stops something — `start`, `stop`, `restart`, `autostart enable|disable` — refuses to run as root, because the socket path and `~/Library/LaunchAgents` are both keyed on the invoking user and root's are not yours; the two `status` verbs are read-only, run as whoever invokes them, and say so when that is root. `launchctl disable` is never issued — it writes a persistent override that survives reinstalling the plist — so the verbs only ever clear one. The Windows port carries the same `autostart` verb and its own `start|stop|restart|status` in `src/app/process_control.cpp` ([WINDOWS-PORT.md](WINDOWS-PORT.md) §5).
- Daemon boot: `NSApplication` with `.accessory` policy, then in `applicationDidFinishLaunching` the instance lock **first** — bind the socket (a node something is listening on — `SocketClient.isListening`, a connect, not a `--ping`, which a daemon mid-config-run cannot answer — means already running: exit non-zero; a dead node is recycled; a node this process did not bind is never unlinked) — then `BarManager.begin()` (bar per display), scheduler / event-bus / provider / mouse wiring, the 1 Hz routine timer, and the config run. A request that lands on the socket before that point waits on the accept thread.
...
**Principle: public API only — nothing private linked or dlsym-resolved.** (Historical: v1 planned a protocol slot for private capabilities with public fallbacks; no such module was ever built and the rule is now absolute for windowing. The engine's single undocumented read — the Wi-Fi scan's `_isPersonalHotspot` ivar peek, §1's table — lives in a provider, not here.)
...
- **Redraw policy**: no loop, ever. At rest the bar is damage-driven: model changes set a dirty flag → one coalesced frame on the next main-queue turn. `NSView.displayLink` (CADisplayLink, macOS 14+) runs **only while animations are active or a marquee demands continuous frames**, and is invalidated on the last removal. While it runs, each tick applies the interpolated values and completions, then paints once, right in the callback; that paint consumes the dirty flag, so the damage those updates raised is retired by the tick instead of painting the same sample a turn late (or twice). The link asks for the slowest hosted panel's rate, floored at 60 and capped at 120 Hz: the tick paints every panel, and a 60 Hz layer driven at 120 would fill its three-drawable pool and stall `nextDrawable()` on the main thread. `--animate` durations stay frames-at-60 Hz converted to seconds, so a faster link draws more frames of the same wall-clock duration, not a faster animation. Acceptance: `powermetrics -s gpu_power` shows ~0% GPU residency when idle; Metal System Trace shows zero command buffers between updates.
- **Shapes**: instanced unit quads, vertex-pulled; fragment shader evaluates analytic SDF: per-corner-radius rounded rect, border via `abs(d+w/2)−w/2`, squircle via superellipse exponent (`cornerExponent`: 2 = circular, ≈4.5 = continuous), shadows as a second quad behind the plate — a hard offset copy by default, and with `shadow.blur > 0` the quad is grown by the blur on every side, carries the true half size in `fill2.xy` and the blur radius in `gradientDir.x`, and flag bit 4 (`flagShadow`) turns the AA edge into a squared-smoothstep falloff of the SDF (no blur passes; a light colour at distance 0 is a glow) — 2-stop gradients interpolated in linear space. Coverage AA via `fwidth`/`smoothstep`; `rasterSampleCount = 1`, no MSAA. Item origins and 1px borders snap to the device pixel grid while the item is still, and are placed at their true value while it has an animation in flight — snapping quantizes travel (a 3pt lift has six pixels to land on at 2x, so it showed the same few pictures at 60 Hz and at 120), and an SDF edge off the grid is antialiased analytically rather than resampled. A quad's size comes from its rounded edges, not its rounded extent, so a pill of constant width does not breathe by a pixel as it slides.
- **Text**: CoreText shaping (`CTLineCreateWithAttributedString` → runs; free font fallback incl. CJK) → glyph raster atlas: `r8Unorm` page for monochrome masks (tinted in-shader), `bgra8Unorm` page for color (emoji, multicolor SF Symbols). Shelf bin-packing; cache keys bucket sizes to quarter points so an animated font size does not mint a glyph per frame (the packer never reclaims); baseline always pixel-snapped, and the horizontal pen with it — except for a marquee, whose pen keeps its true value because snapping a scroll of about two device pixels per 120 Hz frame quantizes it into 2,2,3 steps; `icon.shadow`/`label.shadow` are a second copy of the glyph run at the offset, drawn before the ink. SF Symbols rasterized from `NSImage(systemSymbolName:)` with symbol configuration into the mask atlas → animatable tint. Shaped-line cache keyed by (string, font, size). (Historical: the optional stem-darkening knob was never added.) Fonts parsed as `"Family:Style:Size"` (sketchybar compat) via `CTFontDescriptor`.
...
- **GPU structs** (16-byte aligned): `QuadInstance` {origin, size, radii(4), fill, fill2, gradientDir, borderWidth, cornerExponent, borderColor, flags} — 112 bytes; `GlyphInstance` {origin, size, uvOrigin, uvSize, color, flags} — 64 bytes; `ShapeVertex` 32 bytes; `HoleInstance` (`background.clip` cutouts) 48 bytes; `Uniforms` {viewportSize, holeCount, pad, pointer} — 24 bytes, the pad keeping `pointer` at offset 16 where the Metal struct reads it. Flag bits, strides and every field offset are pinned by `InstancesTests` and re-checked at renderer init (`InstanceLayout.mismatch()` — a drift is a logged startup failure, not a trap). The Windows port shares the layout byte-for-byte except `Hole`, which is 32 bytes there.
...
**Local verbs** (bare words, never `--`-prefixed, so they cannot collide with the message grammar; decided in the client):

```
ybar start [-c <path>]          ybar stop           ybar restart [-c <path>]
ybar status                     ybar autostart enable [-c <path>]|disable|status
ybar theme list|current|use <name>|reset|install <git-url>
```

**Exit codes** (the local verbs above): `0` success, including every idempotent no-op ("already running", "autostart is already disabled"); `1` the operation failed; `2` the invocation was wrong. Message-grammar errors — an unknown `--domain`, a malformed `key=value` — come back as `[!]` replies and exit 1. `ybar --ping` is the scriptable liveness probe (0 alive, 1 not); `ybar status` is for humans and always exits 0.
...
Sources/YBarKit/
  App/        Daemon (boot, wiring, config run, reload), LocalVerbs (`ybar start|stop|restart|status`, `ybar theme`,
              `ybar autostart`; app-bundle discovery, the LaunchAgent plist, the launchctl front end), JSONCConfig
...
Tests/YBarKitTests/               — layout, wire format, property parsing, curves, command grammar, scene output, Lua end-to-end,
                                    ...
                                    process control (bundle discovery, the LaunchAgent plist and its KeepAlive shapes,
                                    `launchctl print` parsing, verb argv, theme discovery)
```
